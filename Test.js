...
function aStarSearch(graph, start, goal, heuristics, timeout) {
    ...
    function nodePriorityComparer(a, b) {
        var aCost = toNodeCost.getValue(a) + nodeToGoalEstCost.getValue(a);
        var bCost = toNodeCost.getValue(b) + nodeToGoalEstCost.getValue(b);
        return bCost - aCost;
    }
    ...
    var previousNode = new collections.Dictionary();
    //List of visited/evaluated nodes
    var visitedNodes = new collections.LinkedList();
    //List of discovered nodes
    var discoveredNodes = new collections.LinkedList();
    //initialising stuff
    ...
    while (!pQueue.isEmpty()) {
        ...
        var currentNeighbourEdges = graph.outgoingEdges(currentNode);
        for (var i = 0; i < currentNeighbourEdges.length; i++) {
            var iNeighbour = currentNeighbourEdges[i].to;
            ...
            var thisPathCost = toNodeCost.getValue(currentNode) + currentNeighbourEdges[i].cost;
            if (discoveredNodes.contains(iNeighbour, nodeEquals)) {
                ...
                var oldPathCost = toNodeCost.getValue(iNeighbour);

...
(function (AStar) {
    ...
    function AStarSearch(start, checkGoal, h, cost, adj) {
        ...
        frontier.enqueue(new Node(start));

        while (!frontier.isEmpty()) {
            var cur = frontier.dequeue();
            if (checkGoal(cur.node)) {
                var finalPath = recons_path(cur);
                return finalPath;
            }
            var adjacentNodes = adj(cur.node);
            adjacentNodes.forEach(function (node) {
                var newCost = costSoFar.getValue(cur.node) + cost(cur.node, node);
...
(function (Planner) {
    ...
    function planInterpretation(intprt, state) {
        ...
        var actions = [];
        var initState = new State(state.stacks, state.holding, state.arm, "");
        var plan = AStar.AStarSearch(initState.copy(), goalFunc, h, costFunc, adjacent);
        console.log(plan);
    ...
    var State = (function () {
        function State(stacks, holding, armpos, action) {
            this.stacks = stacks;
        ...
        State.prototype.copy = function () {
            return new State(this.stacks.map(function (stack) {
                return stack.slice();
            }).slice(), this.holding, this.armpos, this.action);
        };
        return State;
    })();

    function costFunc(a, b) {
        return 1;
    }
    ...
    function makeGoalFunc(intprt) {
        return function (s) {
            var flag = false;
            intprt.forEach(function (i) {
                if (compareState(s, i[0]))
                    flag = true;
            });
            return flag;
        };
    ...
    function compareState(s, lit) {
        ...
        if (lit.rel === "leftof") {
            var stackIndex;
            s.stacks.forEach(function (stack) {
        ...
        if (lit.rel === "inside") {
            s.stacks.forEach(function (stack) {
                if (stack.indexOf(lit.args[1]) >= 0 && stack.indexOf(lit.args[0]) >= 0 && stack.indexOf(lit.args[0]) > stack.indexOf(lit.args[1]) && stack.indexOf(lit.args[0]) - stack.indexOf(lit.args[1]) == 1)
                    flag = true;
            });
        }
        ...
        if (lit.rel === "ontop") {
            s.stacks.forEach(function (stack) {
                if (lit.args[1] === "floor" && stack.indexOf(lit.args[0]) == 0)
                    flag = true;
                else if (stack.indexOf(lit.args[0]) >= 0 && stack.indexOf(lit.args[1]) >= 0 && stack.indexOf(lit.args[0]) - stack.indexOf(lit.args[1]) == 1)
                    flag = true;
            });
        }
        ...
        if (lit.rel === "under") {
            s.stacks.forEach(function (stack) {
                if (stack.indexOf(lit.args[1]) >= 0 && stack.indexOf(lit.args[0]) >= 0 && stack.indexOf(lit.args[0]) < stack.indexOf(lit.args[1]))
                    flag = true;
            });
        }
    ...
    function validatePhysics(holding, support) {
        var objHolding = worldObjs[holding];
        var objSupport = worldObjs[support];

        if (objHolding.form === "ball" && objSupport.form !== "box")
            return false;
        if (objSupport.form === "ball")
            return false;
        if (objHolding.size === "large" && objSupport.size === "small")
            return false;
        if (objSupport.form === "box") {
            if (objHolding.size == objSupport.size && (objHolding.form === "pyramid" || objHolding.form === "plank" || objHolding.form === "box"))
                return false;
        }
        if (objSupport.form === "pyramid" && objHolding.form === "box")
            return false;
        if (objHolding.form === "box" && objHolding.size === "small" && objSupport.form === "brick")
            return false;
    ...
    function interactive(world) {
        function endlessLoop(utterance) {
            ...
            var nextInput = function () {
                return world.readUserInput(inputPrompt, endlessLoop);
            };
            if (utterance.trim()) {

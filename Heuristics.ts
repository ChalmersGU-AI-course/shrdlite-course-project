...
//This module contains the various heuristics that are possible given different pddl goals.
module Heuristics{

    //The penalty used when items are ontop of other items
    //that needs to be accessable to the robot arm.
    var penaltyModulator = 5;

    //From the inputted pddl goal selects a good heuristic specialized for that goal.
    ...
    function ontopHeuristic(over : string, under : string, start : WorldState) : (a : WorldState) => number
    {
        ...
        //The floor is always a special case, since it isn't a stackable object
        if(under === "floor")
        {
            return function( a : WorldState) : number
            {
        ...
        else if(underPos.x == overPos.x)
        {
            ...
            //It can lead to it having to creating multiple stacks. During the block moving.
            ...
            return function( a : WorldState) : number
            {
                ...
                if(a.holding === over)
                {
                    if(underPos.y + 1 == a.stacks[underPos.x].length)
                        points -= penaltyModulator; //We want to encourage the algorithm to pick up the over object.
                }
                else if(a.holding === under)
                {
                    //We discourage that the under object is picked up.
                    points += penaltyModulator;
                }

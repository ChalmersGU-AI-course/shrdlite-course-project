var Search;
(function (Search) {
    function zeroHeuristic(node) {
        return 0;
    }
    Search.zeroHeuristic = zeroHeuristic;
})(Search || (Search = {}));
...
var Search;
(function (Search) {
    function aStar(heuristic, nodeShow) {
        return function (neighbours, start, end) {
            var h = heuristic || Search.zeroHeuristic;
            // "Open set", the nodes that should be evaluated.
            var open = new collections.PriorityQueue(itemCompareFunction);
            // Information associated with each node.
            var info = new collections.Dictionary(nodeShow);
    ...
    Search.aStar = aStar;
    function buildPath(info, node) {
        var ns = [];
        var n = node;
        var i = info.getValue(n);
        while (i.parent) {
            ns.push(n);
            n = i.parent;
            i = info.getValue(n);
    ...
    function itemCompareFunction(a, b) {
        return b[1] - a[1];
    }
})(Search || (Search = {}));
...
///<reference path="../lib/collections"/>
var D = collections.Dictionary;
var N = (function () {
    function N(value, neighbours) {
        if (neighbours === void 0) { neighbours = []; }
        this.value = value;
...
//               |       /
//              (n6)-----
var ns = function (node) { return node.neighbours; };
var end = function (node) { return node.value == "n6"; };
var aStar = Search.aStar;
var s = aStar(Search.zeroHeuristic, function (node) { return node.value; });
var p = s(ns, n1, end);
console.log(n1);
console.log(n2);
console.log(n3);

...
module PlannerTest {
    ...
    match(goal: WorldState) {
      for (var i = 0; i < stacks.length; i++) {
        for (var j = 0; j < stacks[i].length; j++) {
           if (stacks[i][j].form != goal.stacks[i][j].form
               || stacks[i][j].size != goal.stacks[i][j].size
               || stacks[i][j].color != goal.stacks[i][j].color)
             return false;
        }
      }
      return true;
    ...
    expand() {
      neighbours: [WorldState];
      if (crane == null) {
        for (var i = 0; i < stacks.length; i++) {
          if (stacks[i].length == 0)
            continue;
          topObject : WorldObject = this.stacks[i][this.stacks[i].length - 1];
          
          newWorld : WorldState = this.clone();
          newWorld.stacks[i].splice(this.stacks[i].length - 1, 1); 
          newWorld.crane = topObject;
          neighbours.push(newWorld);
        }
      } else {
        for (var i = 0; i < stacks.length; i++) {
          newWorld : WorldState = this.clone();
    ...
    constructor(stacks: [[String]], crane: String) {
      for (var i = 0; i < stacks.length; i++) {
        for (var j = 0; j < stacks[i].length; j++) {
          this.stacks[i].push(new WorldObject(exampleWorldDescription[stacks[i][j]]));
        }
      }
      this.crane = new WorldObject(crane);
      this.h = 0;
    }

    clone() : WorldState {
      braveNewWorld : WorldState = new WorldState();

    ...
    function AStarSearch(start, checkGoal, h, cost, adj) {
        ...
        while (!frontier.isEmpty()) {
            ...
            if (checkGoal(cur)) {
                ...
                var finalPath = recons_path(cameFrom, cur);
                return finalPath;
            }
            var adjacentNodes = adj(cur);
            adjacentNodes.forEach(function (node) {
                var newCost = costSoFar.getValue(cur) + cost(cur, node);
...
(function (Planner) {
    ...
    function planInterpretation(intprt, state) {
        ...
        var bla = AStar.AStarSearch(copyState(initState), goalFunc, h, costFunc, adjacent);
        console.log(bla);
        bla.forEach(function (elem) {
            x.push(elem.action);
        });
        return x;
    }
    ...
    */
    ...
    //drop function
    function drop(state, lastA) {
        if (state.holding != null || lastA == "d" || lastA == "p" || (state.stacks[state.armpos].length != 0 && !Interpreter.checkSize(state.objects[state.holding], state.objects[state.stacks[state.armpos][state.stacks[state.armpos].length - 1]]))) {
            return null;
        }
        var ns = copyState(state);

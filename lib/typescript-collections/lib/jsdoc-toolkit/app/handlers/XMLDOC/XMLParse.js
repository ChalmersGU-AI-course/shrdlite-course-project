...
/**
 * XML Parser object.  Returns an {@link #XMLDOC.Parser.endNode} which is
...
/**
 * A endNode in an XML Document.  Node types are ROOT, ELEMENT, COMMENT, PI, and TEXT.
 * @param parent {XMLDOC.Parser.node} The parent endNode
 * @param name {String} The endNode name
...
XMLDOC.Parser.parse = function(src)
{
   ...
   // The document is the root endNode and cannot be modified or removed
 ...
 * directly.
 * @param parentNode {XMLDOC.Parser.node} The endNode which is the parent of this fragment
      ...
      else if (reStartTag.test(tag[0]))
      {
         ...
         else
         {
            ...
            if (!htmlParsed)
            {
               ...
               // Append the new element endNode
               parentNode.nodes.push(elem);
